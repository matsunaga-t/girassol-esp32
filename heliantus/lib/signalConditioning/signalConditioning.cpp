...
bool LDRNormalizer::calibrate(float refIlluminance1, float refIlluminance2, float rawVoltage1, float rawVoltage2){
    if(rawVoltage1 && rawVoltage2 && refIlluminance1 && refIlluminance2){
        this->alpha = logf(refIlluminance2 / refIlluminance1) / logf( (3.3f / rawVoltage2 - 1) / (3.3f / rawVoltage1 - 1));
        this->beta = refIlluminance1 / powf(3.3f / rawVoltage1 - 1, this->alpha);
        return true;
    }
    return false;
}
